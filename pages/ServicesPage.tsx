    ...
    <div className="pt-48 bg-white text-black min-h-screen pb-40">
      <section className="px-6 md:px-12 max-w-[1800px] mx-auto mb-32">
        ...
        <h1 className="font-heading text-7xl md:text-[10vw] font-bold tracking-tighter uppercase leading-[0.8] mb-12 text-[#165a72]">
          Our <br /><span className="text-[#ed1c24]/10 italic">Services</span>
        </h1>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-black/5">
          {SERVICES.map((s, i) => (
            <div key={s.id} className="p-16 bg-white hover:bg-[#165a72]/5 transition-all group min-h-[500px] flex flex-col justify-between">
              <div>
                <span className="text-[10px] font-bold text-[#165a72]/20 mb-12 block uppercase tracking-widest">Division / 0{i + 1}</span>
              ...
              <div className="flex flex-wrap gap-2">
                {s.features.map(f => (
                  <span key={f} className="text-[9px] font-bold uppercase tracking-[0.2em] px-5 py-2 border border-black/5 rounded-full text-black/30 group-hover:border-[#165a72]/20 group-hover:text-[#165a72] transition-colors">{f}</span>
                ))}
              </div>
            </div>
          ))}
